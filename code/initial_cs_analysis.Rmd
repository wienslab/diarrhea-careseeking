...
```{r}
#TODO: consider pulling non-primary dataset here; would need to double check that age-stratified entries flagged in this collumn no not overlap
...
df_age <- df %>%
  ...
  group_by_at(c('entry_id', 'study_id', 'country_iso3', 'region', 'hic', 'income_group', vars)) %>%
  summarize(n_did_survey = n_did_survey,
            n_did = ifelse(mult_choice == 0, sum(n_did), max(n_did))) %>%
  ungroup() %>%
  unique() %>%
...
```{r}
#TODO: use metafor to look at proportion seeking care overall (either in this subset or in full dataset, or both)
#TODO: use metafor or our logistic regression to test for difference in proportions for children and adults
```
...
We have data from 10 studies that report healthcare seeking at a hospital/clinic by age groups under/over five
...
df_age_strat <- df_age %>%
  ...
  filter(study_id != 49) %>%
  # fill age age L and R NA values
  mutate(age_L = ifelse(is.na(age_L), 0, age_L),
         age_R = ifelse(is.na(age_R), 99, age_R)) %>%
  # classify age cats as either under or over 5 (regardless of proportion)
  mutate(under_five = ifelse(age_R <= 5, 1, 0)) %>%
  # aggregate by under/over five
  group_by_at(c('study_id', 'country_iso3', 'region', 'hic', 'income_group', 
                'recall_time', 'case_cat', 'under_five')) %>%
  summarize(n_did_survey = sum(n_did_survey), 
            n_did = sum(n_did)) %>%
  ungroup() %>%
  unique() %>%
