...
pacman::p_load(
  countrycode, here, readxl, lubridate, tidyverse, ggplot2, doParallel, boot,
...
1) Look at care seeking in the raw data by:
    ...
    +   Outbreak setting
    +   HIC vs. LMIC or geographic region [still need to do]
    +   Age and sex [still need to do; will be a sub-analysis as we don't have this for all studies]

2) Estimate proportion seeking care at hospital or clinic overall, following methods from Wiens et al PLOS Med
3) Examine factors associated with care seeking at a hospital or clinical
...
vars <- c('time_care', 'recall_time', 'pop_cat', 'mult_choice', 'self_or_child',
          'case_cat', 'outbreak_desc', 'location_desc', 'age_L', 'age_R', 'prop_five', 'strat_age')
...
ggplot(df_cor, aes(y=propseek, x = prop_five)) +
...
We have data from 11 studies that report healthcare seeking at a hospital/clinic by age groups
...
df_age_strat <- df_age %>%
  ...
  ungroup() %>%
  filter(n_strats > 1) %>%
  # classify age cats as either under or over 5 (regardless of proportion)
  #TODO: need to think more about how to group and then code these; e.g., what do do when they do under vs over 15
  mutate(strat_u5 = ifelse(age_R < 5, 1, 0),
         strat_o5 = ifelse(age_L >= 5, 1, 0))
```
...
stan_mod_eqns <- list(
  'mod1' = '1',
  'mod2' = 'time_care + mult_choice + case_cat',
  'mod3' = 'time_care + mult_choice'
)
```
...
strata1_dat <- stan_dat %>%
  group_by(time_care, mult_choice, case_cat) %>%
  ...
  ungroup() %>%
  arrange(time_care, mult_choice, case_cat)

strata2_dat <- stan_dat %>%
  group_by(time_care, mult_choice) %>%
  summarize(weighted_mean = round(weighted.mean(n_did/n_did_survey, n_did_survey, na.rm=TRUE)*100,1),
            median = round(median(n_did/n_did_survey)*100,1)) %>%
  ungroup() %>%
  arrange(time_care, mult_choice)
```
...
cats_mod2 <- obs_by_cat(stan_dat, c('time_care', 'mult_choice', 'case_cat'))
cats_mod3 <- obs_by_cat(stan_dat, c('time_care', 'mult_choice'))
...
prop_mod2_sub <- prop_strat_mod2 %>%
  filter(`time_careFirst source` == 0 & `time_carePrior to current visit` == 0 & mult_choice == 1)
...
## Additional things to potentially consider
