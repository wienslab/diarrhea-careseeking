...
1.  Unadjusted
2.  Adjusted for study methodology and population:
    -   study type
    -   timing of care seeking
    -   recall period
    -   multiple choice questions
    -   study population
3.  Adjusted for study methodology and contextual factors:
    -   study type
    -   timing of care seeking
    -   recall period
    -   multiple choice questions
...
*NB / TODO*: \* Before building models 2 and 3, we need to run univariate analyses. Results below just represent unadjusted, model 1 results for now. \* Also need to examine why proportion seeking care is so sensitive to priors. May need to reconstruct this model as well. \* Structure this code so it requires less copy/paste (will get cumbersome with stratifications and sub-analyses).
...
file_names <- file_names[grep('dia', file_names)]
...
prop_mod1 <- stratify('mod1', 'dia', data.frame(alpha = 1, n = 1, prop = 1))[[1]]
...
propseek1 <- readRDS(here::here('data', 'generated_data', 'care_seeking_estimates', 'propseek-dia-mod1.rds'))

s4aL <- plot_prior_post(propseek1$alpha$alpha, 
                'logit(global intercept)',
                arg1=0, arg2=5, dprior='dnorm') +
  ggtitle('Unadjusted')
...
tabS4 <- data.frame('Version' = c('Unadjusted'),
                    'Proportion' = paste(pr2[,2],'(',pr2[,1],'-',pr2[,3],')'))
