...
author: "Kirsten E. Wiens, Marissa H. Miller, Daniel J. Costello, Ashlynn P. Solomon, Skye M. Hilbert, Andrea G. Shipper, Elizabeth C. Lee, Andrew S. Azman"
output: 
  html_document:
    toc: true
    toc_float: true
...
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE,
                      dev=c('png', 'pdf', 'tiff'), ppi=300, fig.path='figures/')
...
df <- df %>%
  ...
  mutate(self_or_child = ifelse(self_or_child == 'Other (describe in notes)', 'Combination', self_or_child)) %>%
  ...
  # create variable for whether data collection was done at a clinic vs. in the community
  mutate(pop_clinic = ifelse(pop_cat %in% c('Caregiver of patient at health facility', 'Patient at health facility'), 1, 0)) %>%
...
covs <- c('time_care', 'recall_time', 'mult_choice', 'self_or_child',
...
sr_dat <- df %>%
  ...
  group_by_at(c('entry_id', 'study_id', 'country_iso3', 'region', 'hic', 'income_group', 'pop_clinic',
  ...
  group_by_at(c('study_id', 'country_iso3', 'region', 'hic', 'income_group', 'pop_clinic',
                'ic_combined', 'sample_justified', 'nonresponse', 'quality_score', covs)) %>%
...
sr_dat_would <- df_would %>%
  # restrict to LMICs
  filter(hic == 0) %>%
  # restrict to non-clinic-based surveys
  filter(pop_clinic == 0) %>%
  # filter to hospital/clinic (not private)
  filter(location_type %in% c('Hospital/Clinic', 'Public Hospital/Clinic')) %>%
  # aggregate care seeking to entry_id, accounting for multiple choice questions
...
sr_dat_sources <- df %>%
  ...
  mutate(propseek = n_did/n_did_survey,
         location_type = factor(location_type, 
                                levels = c('Any care outside home', 'Hospital/Clinic', 'Private Hospital/Clinic',
                                           'Other', 'Pharmacy', 'Health post', 'CHW', 'Traditional Healer', 'Friends/Family')))
```

```{r clinic-based data for did analysis of all care sources}
# create secondary dataset for clinic-based studies: where else DID people seek care

# proportion seeking care for examining non-hospital sources of care
# aggregate across stratifications that we're not including as covariates or stratifications
sr_clinic_sources <- df %>%
  # restrict to LMICs
  filter(hic == 0) %>%
  # restrict to clinic-based surveys
  filter(pop_clinic == 1, location_type != 'Any care outside home') %>%
  # group public hospital/clinic with general hospital/clinic for consistency with rest of paper
  mutate(location_type = ifelse(location_type == 'Public Hospital/Clinic', 'Hospital/Clinic', location_type)) %>%
  # aggregate care seeking to entry_id, accounting for multiple choice questions
  group_by_at(c('entry_id', 'study_id', 'country_iso3', 'region', 'hic', 'income_group', 'location_type', covs)) %>%
  ...
  mutate(propseek = n_did/n_did_survey,
         location_type = factor(location_type, 
                                levels = c('Hospital/Clinic', 'Private Hospital/Clinic',
                                           'Other', 'Pharmacy', 'Health post', 'CHW', 'Traditional Healer', 'Friends/Family')))
```
...
![PRISMA diagram.](PRISMA_20241119.png)

<br>

### Figure S1. Data coverage by geography

Number of observations in the primary dataset at each administrative level as defined by GADM (https://gadm.org/) by country. Countries with >10 observations displayed as 10.
...
Number of observations in the primary dataset at each administrative level as defined by GADM (https://gadm.org/) by country within different time periods. Countries with >10 observations displayed as 10. Year represents the year sampling was completed. Excludes 6 studies missing a study end date.
...
Number of observations in the primary dataset with each characteristic. A study has more than one observation if it reported results stratified by any of these characteristics. 
...
schar <- df %>% 
  ...
  mutate(recall_time = ifelse(recall_time < 14, '2-7 days',
         ...
         pop_clinic = ifelse(pop_clinic == 1, 'Yes', 'No')) %>%
  # sum up number of tests and sample sizes by observation
  dplyr::select_at(c('study_id', 'country_iso3', 'income_group', 'pop_clinic', covs)) %>%
  unique() %>%
...
schar %>%
  ...
  mutate(variable = ifelse(variable == 'Pop clinic', 'Clinic-based surveillance',
                           ifelse(variable == 'Case cat', 'Case definition',
                                  ifelse(variable == 'Outbreak desc', 'During/after diarrhea outbreak', variable)))) %>%
  pretty_table()
...
**Note:** "Hospitals or clinics" excludes individuals seeking care exclusively or explicitly at private hospitals/clinics, i.e., this includes location categories "Hospital/Clinic" (general) and "Public Hospital/Clinic" (more specific). For multiple choice questions where there is more than one category of hospital/clinic, we take the largest proportion that sought care across them.

In addition, with the exception of Figure S8, clinic-based surveillance studies are excluded from all figures and analyses below.
...
df_hosp <- sr_dat %>%
  ...
  mutate(region = ifelse(region %in% c('Eastern Africa', 'Middle Africa', 'Southern Africa', 'Western Africa'), 
         ...
         outbreak_desc = ifelse(outbreak_desc == 1, 'Yes', 'No'),
         pop_clinic = ifelse(pop_clinic == 1, 'Yes', 'No')) %>%
  # remove clinic-based studies
  filter(pop_clinic == 'No')

# just studies in LMIC
df_hosp_all_lmic <- df_hosp_all %>% 
  filter(hic == 0)

# just non-clinic-based studies in LMIC
df_hosp_lmic <- df_hosp %>% 
  filter(hic == 0)

# just studies in HIC
df_hosp_all_hic <- df_hosp_all %>% 
  filter(hic == 1)

# just non-clinic-based studies in HIC
df_hosp_hic <- df_hosp %>% 
  filter(hic == 1)
```
...
Proportion of respondents that reported seeking care for diarrheal illness at a hospital or clinic grouped by A) geographic region and B) geographic region and case definition. Each point represents an observation. Boxes represent the median and interquartile range (IQR) of the proportion for each group. Lines extend from the top and bottom of box to the largest proportion value no further than 1.5 * IQR from the box.
...
Proportion of respondents that reported seeking care for diarrheal illness at a hospital or clinic grouped by A) World Bank income group and B) World Bank income group and case definition. Each point represents an observation. Boxes represent the median and interquartile range (IQR) of the proportion for each group. Lines extend from the top and bottom of box to the largest proportion value no further than 1.5 * IQR from the box.
...
Proportion of respondents that reported seeking care for diarrheal illness at a hospital or clinic grouped by A) whether the study surveyed participants about care seeking at any time during illness or their first source of care, B) survey respondent category, C) whether or not the study described an outbreak, and D) whether the care seeking was for the respondent themselves, their child, or other, which often included any household member. Each point represents an observation (N = 145). Boxes represent the median and interquartile range (IQR) of the proportion for each group. Lines extend from the top and bottom of box to the largest proportion value no further than 1.5 * IQR from the box.

```{r}
f2a <- plot_seekprop(df_hosp_lmic, 'time_care', num_colors=2) +
...
```{r}
f2b <- plot_seekprop(df_hosp_lmic, 'case_cat', num_colors=3) +
...
f2c <- plot_seekprop(df_hosp_lmic, 'outbreak_desc', num_colors=2) +
  ggtitle('During or following a diarrhea outbreak')
...
f2d <- plot_seekprop(df_hosp_lmic, 'self_or_child', num_colors=3) +
  ggtitle('Relationship of diarrhea case to survey respondent')
...
Proportion of respondents in low- and middle-income countries (LMICs) that reported seeking care for diarrheal illness at a hospital or clinic grouped by A) overall quality score, B) whether the data is internally consistent (i.e., numbers for the proportion seeking care were identical throughout the abstract, text, and tables), C) whether the manuscript included a justification for their sample size, and D) whether the non-response rate was reported. Overall quality score is the sum of B-D. Each point represents an observation. Boxes represent the median and interquartile range (IQR) of the proportion for each group. Lines extend from the top and bottom of box to the largest proportion value no further than 1.5 * IQR from the box.
...
A-B) Relationship between the proportion of respondents that reported seeking care for diarrheal illness at a hospital or clinic (y-axis) and the proportion of the study population under five years of age (x-axis), A) overall and B) stratified by diarrhea case definition. Size of points is proportional to the number of survey respondents per observation (N = 134). Spearman’s rho is shown with 95% confidence interval (CI) in green. Linear relationships are shown with purple lines. C-D) Proportion seeking care grouped by studies that were exclusively among children under five (N = 98) or exclusively among individuals over five (N = 22), C) overall and D) stratified by diarrhea case definition.

```{r}
# set variables to keep
vars <- c('time_care', 'recall_time', 'pop_clinic', 'mult_choice', 'self_or_child',
          'case_cat', 'outbreak_desc', 'location_desc', 'prop_five', 'strat_age')

df_age <- df %>%
  # filter to hospital/clinic (not private), LMICs, and non-clinic-based surveillance
  filter(location_type %in% c('Hospital/Clinic', 'Public Hospital/Clinic'),
         hic == 0,
         pop_clinic == 0) %>%
  # aggregate care seeking to entry_id, accounting for multiple choice questions
...
Proportion of respondents that reported seeking care for diarrheal illness at all categories of care sources, including any care outside of the home, hospitals or clinics including public facilities, private hospitals or clinics, pharmacies or kiosks, health posts, community healthcare workers (CHW), traditional healers, and friends or family, for A) all studies and B) the subset of studies that asked survey respondents about the first source of care they sought. Each point represents an observation.
...
f4a <- plot_seekprop(sr_dat_sources, 'location_type',
  ...
  theme(legend.position = 'none') +
  scale_color_manual(values=c('black', brewer.pal(n=9, name='Dark2'))) +
  ggtitle('Overall (any care or first source)')
...
Proportion of respondents in LMICs that reported seeking care for diarrheal illness at all categories of care sources by the World Bank income group of the country the study was conducted in. Each point represents an observation. Boxes represent the median and interquartile range (IQR) of the proportion for each group. Lines extend from the top and bottom of box to the largest proportion value no further than 1.5 * IQR from the box.
...
Proportion of respondents in LMICs that reported seeking care for diarrheal illness at all categories of care sources by diarrhea case definition. Each point represents an observation. Boxes represent the median and interquartile range (IQR) of the proportion for each group. Lines extend from the top and bottom of box to the largest proportion value no further than 1.5 * IQR from the box.
...
### Figure S8. Proportion that sought care by source for clinic-based surveillance studies

Proportion of respondents in clinic-based surveillance studies in LMICs that reported seeking care for diarrheal illness at all categories of care sources A) as their first source of care and B) prior to their current hospital or clinic visit. Each point represents an observation. Boxes represent the median and interquartile range (IQR) of the proportion for each group. Lines extend from the top and bottom of box to the largest proportion value no further than 1.5 * IQR from the box.

```{r figS8, fig.width = 7, fig.height = 6}
# overall proportion seeking care
fS8a <- plot_seekprop(sr_clinic_sources %>% filter(time_care == 'First source'), 
                      'location_type', num_colors=9, palette = 'Spectral') +
  theme(legend.position = 'none') +
  scale_color_manual(values=brewer.pal(n=9, name='Dark2'), drop = FALSE) +
  ggtitle('First source of care among people surveyed at hospitals/clinics')

# first source of care
fS8b <- plot_seekprop(sr_clinic_sources %>% 
                        filter(time_care == 'Prior to current visit'), 
                     'location_type', num_colors=9, palette = 'Spectral') +
  theme(legend.position = 'none') +
  scale_color_manual(values=brewer.pal(n=9, name='Dark2'), drop = FALSE) +
  ggtitle('Care sought prior to current hospital/clinic visit')

cowplot::plot_grid(fS8a, fS8b,
                   labels = c('A', 'B'),
                   nrow = 2)

# sr_clinic_sources %>% filter(time_care == 'First source') %>%
#   group_by(location_type) %>%
#   summarize(median = median(propseek),
#             N = length(propseek),
#             IQR_L = quantile(propseek, 0.25),
...
# sr_clinic_sources %>% filter(time_care == 'Prior to current visit') %>%
#   group_by(location_type) %>%
#   summarize(median = median(propseek),
#             N = length(propseek),
#             IQR_L = quantile(propseek, 0.25),
...
# sr_dat %>% filter(hic == 1) %>% filter(pop_clinic == 0) %>%
#   summarize(median = median(propseek),
#             N = length(propseek),
#             IQR_L = quantile(propseek, 0.25),
#             IQR_R = quantile(propseek, 0.75)) %>%
...
Estimated proportion that sought care for an actual episode of diarrheal illness at a hospital or clinic across the 145 observations included in the random-effects meta-analysis.
...
Estimated proportion that would have sought care for a hypothetical episode of diarrheal illness at a hospital or clinic across the 29 observations included in the random-effects meta-analysis.

```{r figS10, fig.width = 10, fig.height = 8}
...
Estimated proportion that sought care for an actual episode of diarrheal illness at a hospital or clinic across the 24 observations from high-income countries (HICs) included in the random-effects meta-analysis.

```{r figS11, fig.width = 10, fig.height = 7}
...
mv_fit <- rma(measure = 'PLO',
              xi = n_did, ni = n_did_survey,
              method = 'REML', 
              mods = ~ case_cat + time_care, 
              random = ~ case_cat + time_care | study_id,
              data = meta_dat)
  ...
  # save beta coefficients
  for (c in 2:length(rownames(mv_fit$beta))) {
      mod_ests <- rbind(mod_ests,
                       data.frame(
                         run_id = 'all-mod2',
...
Odds of seeking care for diarrhea at a hospital or clinic in a LMIC for each indicated variable category compared to the reference group, adjusting for no other variables. ** indicates significant (p < 0.05).
...
Odds that an individual seeks care for diarrheal illness for a child under five years of age compared to an adult or individual over five years of age. Sub-analysis of studies only among children or only among adults. ** indicates significant (p < 0.05).
...
Odds that an individual seeks care for diarrheal illness for a child under five years of age compared to an adult or individual over five years of age. Sub-analysis of studies with care seeking stratified for children and adults. ** indicates significant (p < 0.05).
...
Odds that an observation in a LMIC used severe diarrhea or cholera as the case definition instead of general diarrhea for each category compared to the reference group. ** indicates significant (p < 0.05).
...
### Table S4. Sub-analyses of associations with outbreak context and recall period
...
Odds that an individual sought care for diarrheal illness for severe diarrhea or cholera and gastroenteritis or other etiologies compared to general diarrhea, adjusting for timing of care seeking. ** indicates significant (p < 0.05).
...
or_table(c3) %>%
  pretty_table(title = 'Model including timing of care seeking and case definition')
...
Estimated proportion that sought care at a hospital or clinic across the 122 in LMICs studies that used a general diarrhea case definition
...
Estimated proportion that sought care at a hospital or clinic across the 15 studies in LMICs that used a severe diarrhea or cholera case definition, including deaths.
...
Estimated proportion that sought care at a hospital or clinic across the 8 studies in LMICs that used a gastroenteritis or other etiologies case definition.
