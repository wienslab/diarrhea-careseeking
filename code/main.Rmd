...
ma_nobayes <- metaprop(n_did, n_did_survey, data = arrange(meta_dat, propseek), 
                       studlab = paste0(meta_dat$study_id, '_', meta_dat$country_iso3), 
...
```{r}
df_meta2 <- df_meta2 %>%
  mutate(vi = prop_var(propseek, n_did_survey),
         yi = logit(propseek))

mr1 <- rma(yi, vi, mods = ~ prop_five, 
           data = df_meta2, digits = 3)

mr2 <- rma.mv(yi, vi, mods = ~ prop_five, 
              random = ~ prop_five | study_id, struct = 'DIAG',
              data = df_meta2, digits = 3)
```
...
ma_casedef <- metaprop(n_did, n_did_survey, data = arrange(meta_dat, propseek), 
                       subgroup = meta_dat$case_cat,
                       studlab = paste0(meta_dat$study_id, '_', meta_dat$country_iso3), 
                       method = 'GLMM', common = FALSE)
...
mr1 <- rma(yi, vi, mods = ~ case_cat, 
           random = ~ case_cat | study_id,
           data = meta_dat, digits = 3)

mr2 <- rma.mv(yi, vi, mods = ~ case_cat, 
              random = ~ case_cat | study_id, struct = 'DIAG',
              data = meta_dat, digits = 3)
```
