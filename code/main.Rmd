...
# data for clinic-based surveillance analysis
meta_dat_clinic <- sr_dat %>% 
  filter(hic == 0) %>%
  filter(pop_clinic == 1)

meta_dat_clinic <- meta_dat_clinic %>%
  mutate(study_lab = 1:nrow(meta_dat_clinic)) %>%
  arrange(propseek)
...
ma_unadj_clinic <- metaprop(n_did, n_did_survey, data = meta_dat_clinic, 
                     studlab = paste0(meta_dat_clinic$country_iso3, ' (', meta_dat_clinic$study_id, ') ', 
                                      meta_dat_clinic$self_or_child, ', ', meta_dat_clinic$case_cat), 
                     method = 'GLMM', common = FALSE, prediction = TRUE)
...
### Figure S8. Forest plot of care seeking at a hospital or clinic in LMICs
...
```{r figS8, fig.width = 10, fig.height = 31}
...
### Figure S9. Forest plot of hypothetical care seeking at a hospital or clinic in LMICs
...
```{r figS9, fig.width = 10, fig.height = 8}
...
### Figure S10. Forest plot of care seeking at a hospital or clinic by individuals already at a clinic in LMICs

Estimated proportion of individuals that reported that a hospital or clinic was their first source of care or that they had previously sought care at a hospital or clinic across the 6 observations from clinic-based studies in LMICs included in the random-effects meta-analysis.

```{r figS10, fig.width = 10, fig.height = 3}

ma_unadj_clinic

forest(ma_unadj_clinic)
```
