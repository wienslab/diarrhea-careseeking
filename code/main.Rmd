...
df <- df %>%
  ...
  mutate(sampling_method = gsub('\\s*\\([^\\)]+\\)', '', sampling_method),
...
This also excludes multiple choice questions (as these can sum to > 1 for studies that report both general hospital and public hospital).
...
```{r}
df_hosp <- df %>%
  # remove multiple choice questions (for now) %>%
  filter(mult_choice == 0) %>%
...
#### By case definition and study population categories

```{r}
f3a <- plot_seekprop(df_hosp, 'pop_cat', num_colors=8) +
...
```{r}
f3b <- plot_seekprop(df_hosp, 'case_cat', num_colors=8) +
...
```{r fig3, fig.width = 9, fig.height = 9}
