...
stan_mod_eqns <- list(
  ...
  'mod3' = 'study_type + time_care + recall_time + outbreak_desc'
)
```
...
#TODO: figure out why Stan stopped working, then re-run the case_cat univariate and multivariate analyses
#stan_redo <- TRUE
#for (i in c(6)) {
...
strata1_dat <- stan_dat %>%
  group_by(study_type, time_care, recall_time, case_cat) %>%
  ...
  ungroup() %>%
  arrange(study_type, time_care, recall_time, case_cat)

strata2_dat <- stan_dat %>%
  group_by(study_type, time_care, recall_time, outbreak_desc) %>%
  summarize(weighted_mean = round(weighted.mean(n_did/n_did_survey, n_did_survey, na.rm=TRUE)*100,1),
            median = round(median(n_did/n_did_survey)*100,1)) %>%
  ungroup() %>%
  arrange(study_type, time_care, recall_time, outbreak_desc)
```
...
cats_mod2 <- obs_by_cat(stan_dat, c('study_type', 'time_care', 'recall_time', 'case_cat'))
cats_mod3 <- obs_by_cat(stan_dat, c('study_type', 'time_care', 'recall_time', 'outbreak_desc'))
...
prop_mod2_sub <- prop_strat_mod2 %>%
  filter(study_typeSurvey == 1 & recall_timeunder30 == 1 &
...
case_cats <- names(prop_mod2_sub)[grep("pop_cat", names(prop_mod2_sub))]
case_cats <- gsub('pop_cat', '', case_cats)
