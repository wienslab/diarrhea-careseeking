...
# set covariates
#TODO: need to restructure the code to have observations by the final covariates included; for now, leaving the data stratified by urban/rural (if we aggregate above this, we'll have 150 observations)
#TODO: need to set reference categories for the covariates
...
#     time_care = 'Any care'
#     recall_time = 'over30_orNA' for now!
#TODO: update model config so that we can set the recall_time here to under 30 (may need to not have cholera separate)
#TODO: or can do separate model where we look at cholera, but only adjust for timing of care seeking

prop_mod2_sub <- prop_strat_mod2 %>%
  filter(recall_timeunder30 == 0 &
...
Note: Stratifications currently correspond to questions that refer to 1) any care seeking, and 2) have recall periods >30 days. Given the latter, the adjusted estimates are likely overestimates. In order to create these estimates for recall periods <30 days, we'll need to change how we're doing the cholera analysis (we have no studies specific for cholera where a recall period wasn't either missing or very long).

```{r}
#TODO: table like table s4
...
tabs <- data.frame('Model' = c('Unadjusted', 
                               rep('Adjusted for care seeking questions 1', 4),
...
cs_plot <- cs %>% 
  mutate(facet = c('Unadjusted', 
                   rep('Adjusted for questions 1', 4),
