...
#TODO: add model where we just adjust for methods (get an "adjusted" estimate)
#TODO: adjust for multiple choice questions (as a covariate, need to make sure that it's a covariate though)
#TODO: consider converting self_or_child back to categorical, or handling another way eg stratifying (doesn't make sense now)
stan_mod_eqns <- list(
  'mod1' = '1',
  'mod2' = 'time_care + case_cat',
  'mod3' = 'recall_time + self_or_child'
)
```
...
sr_dat <- df %>%
  ...
  mutate(case_cat = ifelse(case_cat %in% c('Giardiasis', 'Other (list etiologies in notes)'), 'Gastroenteritis or non-Vc etiologies', 
         ...
         self_or_child = ifelse(self_or_child == 'Child', 'Child', 'Self or family member')) %>%
...
# loop over covariates
for (cov in c('case_cat', 'time_care', 'outbreak_desc', 'recall_time')) {
  ...
  if (!file.exists(out_est_file) | stan_redo) {
    ...
    seekrate <- run_analysis_stan(
      model_script = model_script,
      dat_surveyed = stan_dat$n_did_survey,
      dat_soughtcare = ifelse(stan_dat$self_or_child == 'Child', 0, 1),
...
# loop over covariates
for (cov in c('case_cat', 'time_care', 'recall_time', 'self_or_child')) {
...
for (cov in c('outbreak_desc', 'time_care', 'recall_time', 'self_or_child')) {
...
strata2_dat <- stan_dat %>%
  group_by(recall_time, self_or_child) %>%
  summarize(weighted_mean = round(weighted.mean(n_did/n_did_survey, n_did_survey, na.rm=TRUE)*100,1),
            median = round(median(n_did/n_did_survey)*100,1)) %>%
  ungroup() %>%
  arrange(recall_time, self_or_child)
```
...
cats_mod3 <- obs_by_cat(stan_dat, c('recall_time', 'self_or_child'))
...
# save estimates for Self or family member
nonout_id <- which(prop_mod3_sub$self_or_child==1)
mod_ests <- rbind(mod_ests,
                  data.frame(run_id = 'Self or family member',
                             stan_id = 'prop',
                             mean = prop_mod3_sub[nonout_id, 'mean'],
                             lower = prop_mod3_sub[nonout_id, 'lower'],
                             upper = prop_mod3_sub[nonout_id, 'upper'],
                             row.names = NULL
                             )
                  )

# save estimates for Child
out_id <- which(prop_mod3_sub$self_or_child==0)
mod_ests <- rbind(mod_ests,
                  data.frame(run_id = 'Child',
                             stan_id = 'prop',
                             mean = prop_mod3_sub[out_id, 'mean'],
                             lower = prop_mod3_sub[out_id, 'lower'],
                             upper = prop_mod3_sub[out_id, 'upper'],
                             row.names = NULL
                             )
                  )
...
So we run two analyses below, looking separately at case_cat and self_or_child
...
2. In the second model, we test self_or_child adjusting for recall_period, assuming that self_or_child is more directly related to care seeking than how the timing of the care seeking question was phrased or whether they had vomiting or other symptoms (this assumption may be less reasonable).

#### Factors associated with variation in care seeking rates

Odds that an individual with diarrhea seeks care for themselves or their child in an outbreak context or by alternate case definitions, adjusting for different ways that the care seeking questions were asked (i.e., timing of care itself and recall period).
...
#TODO: the model 2 results are weird (shouldn't be so high for the 30 day recall period) and weird that child care seeking is lower. may not be appropriate to do this and should also check model convergence
c3 <- mod_ests %>%
...
# make a table
pretty_table(c2, title = 'Model adjusting for timing of care seeking question + case definition')
pretty_table(c3, title = 'Model adjusting for recall period + whether the question is about self/family member of child')
```
...
cs <- mod_ests %>%
  ...
  mutate(run_id = ifelse(run_id == 'all-mod1', '1', run_id))

# table
tabs <- data.frame('Model' = c('Unadjusted', 
                               rep('Stratum: Any care seeking', 3),
                               rep('Stratum: Recall period 30 days', 2)),
                   'Variable' = cs$run_id,
...
cs_plot <- cs %>% 
  mutate(facet = c('Unadjusted', 
                   rep('Stratum: Any care seeking', 3),
                   rep('Stratum: Recall period 30 days', 2))) %>%
  mutate(facet = factor(facet, levels = c('Unadjusted',
                                          'Stratum: Any care seeking',
                                          'Stratum: Recall period 30 days')))
...
```{r}
#TODO: figure like figure 4, grouped by case_cat (maybe also labeled by outbreak, TBD decide after deciding final model structures for multivariate analysis; for example, maybe just label by country and outbreak_desc; might also want to include "adjusted" estimates that account for study methods i.e., mod2 or 3 or a new one)
```
