...
#TODO: examine outbreak within case definitions
stan_mod_eqns <- list(
  'mod1' = '1',
  'mod2' = 'time_care + case_cat',
  'mod3' = 'recall_time + time_care'
)
```
...
strata2_dat <- stan_dat %>%
  group_by(recall_time, time_care) %>%
  summarize(weighted_mean = round(weighted.mean(n_did/n_did_survey, n_did_survey, na.rm=TRUE)*100,1),
            median = round(median(n_did/n_did_survey)*100,1)) %>%
  ungroup() %>%
  arrange(recall_time, time_care)
```
...
cats_mod3 <- obs_by_cat(stan_dat, c('recall_time', 'time_care'))
...
# make a table
pretty_table(c2, title = 'Model adjusting for multiple choice, timing of care seeking question, and case definition')
pretty_table(c3, title = 'Model adjusting for methods: multiple choice, recall period, and timing of care seeking')
```
...
tabs <- data.frame('Model' = c('Unadjusted', 
                               ...
                               rep('Stratum: Any care seeking + recall period 30', 1)),
                   'Variable' = cs$run_id,
