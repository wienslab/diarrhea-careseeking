...
stan_mod_eqns <- list(
  'mod1' = '1',
  'mod2' = 'time_care + recall_time + case_cat',
  'mod3' = 'time_care + recall_time + outbreak_desc'
)
```
...
strata1_dat <- stan_dat %>%
  group_by(time_care, recall_time, case_cat) %>%
  ...
  ungroup() %>%
  arrange(time_care, recall_time, case_cat)

strata2_dat <- stan_dat %>%
  group_by(time_care, recall_time, outbreak_desc) %>%
  summarize(weighted_mean = round(weighted.mean(n_did/n_did_survey, n_did_survey, na.rm=TRUE)*100,1),
            median = round(median(n_did/n_did_survey)*100,1)) %>%
  ungroup() %>%
  arrange(time_care, recall_time, outbreak_desc)
```
...
cats_mod2 <- obs_by_cat(stan_dat, c('time_care', 'recall_time', 'case_cat'))
cats_mod3 <- obs_by_cat(stan_dat, c('time_care', 'recall_time', 'outbreak_desc'))
...
# save estimates for non-outbreak setting
nonout_id <- which(prop_mod3_sub$outbreak_desc==0)
mod_ests <- rbind(mod_ests,
                  data.frame(run_id = 'Non-outbreak',
                             stan_id = 'prop',
...
# save estimates for Outbreak setting
out_id <- which(prop_mod3_sub$outbreak_desc==1)
mod_ests <- rbind(mod_ests,
                  data.frame(run_id = 'Outbreak',
                             stan_id = 'prop',
...
# make a table
pretty_table(c2, title = 'Model adjusting for questions + case definition')
pretty_table(c3, title = 'Model adjusting for questions + outbreak context')
```
...
Care seeking question variables include recall period and timing of care seeking. 

Note: Stratifications currently correspond to questions that refer to 1) any care seeking, and 2) have recall periods <30 days. Also we're probably inappropriately adjusting here given some potential confounding, so this is very prelim!
...
tabs <- data.frame('Model' = c('Unadjusted', 
                               rep('Adjusted for care seeking questions 1', 3),
                               rep('Adjusted for care seeking questions 2', 2)),
                   'Variable' = cs$run_id,
...
cs_plot <- cs %>% 
  mutate(facet = c('Unadjusted', 
                   rep('Adjusted for questions 1', 3),
                   rep('Adjusted for questions 2', 2))) %>%
  mutate(facet = factor(facet, levels = c('Unadjusted',
                                          'Adjusted for questions 1',
                                          'Adjusted for questions 2')))
