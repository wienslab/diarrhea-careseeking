...
We run two analyses below, adjusting for methods and then adjusting for methods and case_cat. We do not adjust for recall period in the case_cat analysis because there was potential collinearity/confounding betweem=n those variables.
...
2. In the second model, we include methods variables: timing of care seeking.
...
Care seeking question variables include recall period and timing of care seeking. Stratifications currently correspond to questions that refer to any care seeking.
...
# - Overall, proportion seeking care gets estimated higher than the data; should we try a prior shifted to the left to see how this changes things/make sure this isn't all just prior-dependent?
...
# forest plot for V cholerae or severe diarrhea
vc_forest <- forest_plot(study_est %>% 
                        filter(case_cat=='V cholerae or severe diarrhea') %>%
                        mutate(id = 1:nrow(study_est %>% filter(case_cat=='V cholerae or severe diarrhea')))) +
  ggtitle('V cholerae or severe diarrhea')
