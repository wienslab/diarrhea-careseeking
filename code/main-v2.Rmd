...
```{r prep data for meta-analysis}
# subset input data to LMICs
meta_dat <- sr_dat %>% filter(hic == 0)
#TODO: add stratified results to Table S7, S11, and S12
case_strat <- 'all'

# add random effect label by observation
meta_dat <- meta_dat %>% mutate(study_lab = 1:nrow(meta_dat))

# data for "would you" analysis
meta_dat_would <- sr_dat_would %>% mutate(study_lab = 1:nrow(sr_dat_would))
...
# did you seek care
ma_unadj <- metaprop(n_did, n_did_survey, data = arrange(meta_dat, propseek), 
                     #TODO: update this label to something more informative
                     studlab = paste0(meta_dat$study_id, '_', meta_dat$country_iso3), 
                     method = 'GLMM', common = FALSE, prediction = TRUE)

# would you seek care
ma_unadj_would <- metaprop(n_would, n_would_survey, data = arrange(meta_dat_would, propseek), 
                           #TODO: update this label to something more informative
                           studlab = paste0(meta_dat_would$study_id, '_', meta_dat_would$country_iso3), 
                           method = 'GLMM', common = FALSE, prediction = TRUE)
```

### Figure S9. Forest plot for unadjusted meta-analysis of whether people sought care at a hospital or clinic for an actual diarrhea episode
...
### Figure S10. Forest plot for unadjusted meta-analysis of whether people would seek care at a hospital or clinic for a hypothetical diarrhea episode
...
```{r run univariate meta-regressions}
...
meta_dat_vc <- meta_dat %>% filter(case_cat != 'Gastroenteritis or non-Vc etiologies')
...
```{r run multivariate meta-analysis}
...
tabS1 <- data.frame('Version' = c('Unadjusted - did you', 
...
mod_ests_soc <- mod_ests %>%
    filter(grepl('all-self_or_child', run_id) & mod_id != 'alpha') %>%
  or_table()
...
mod_ests_out <- mod_ests %>%
    filter(grepl('all-outbreak_desc', run_id) & mod_id != 'alpha') %>%
  or_table()
...
mod_ests_ge <- mod_ests %>%
  filter(grepl('all-ge_nonvc', run_id) & mod_id != 'alpha') %>%
  or_table()
...
# c3 <- mod_ests %>%
#   filter(run_id %in% c('all-mod3'), mod_id != 'alpha')
# 
# # make tables
# or_table(c3) %>%
